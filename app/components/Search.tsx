import { Form, Link, useFetcher, useParams, type FormProps } from '@remix-run/react'
import { Image, Money, Pagination } from '@shopify/hydrogen'
import React, { useEffect, useRef } from 'react'
import { applyTrackingParams } from '~/lib/search'

import type { PredictiveArticleFragment, PredictiveCollectionFragment, PredictiveProductFragment, SearchQuery } from 'storefrontapi.generated'

type PredicticeSearchResultItemImage = PredictiveCollectionFragment['image'] | PredictiveArticleFragment['image'] | PredictiveProductFragment['variants']['nodes'][0]['image']

type PredictiveSearchResultItemPrice = PredictiveProductFragment['variants']['nodes'][0]['price']
 ...
 **/
export function PredictiveSearchForm({ action, children, className = 'predictive-search-form', ...props }: SearchFromProps) {
  ...
  function fetchResults(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    fetcher.submit({ q: newSearchTerm, limit: '6' }, { method: 'GET', action: localizedAction })
  }
...
export function PredictiveSearchResults() {
  const { results, totalResults, searchInputRef, searchTerm, state } = usePredictiveSearch()
 ...
 * ```
 */
function pluralToSingularSearchType(type: NormalizedPredictiveSearchResults[number]['type'] | Array<NormalizedPredictiveSearchResults[number]['type']>) {
  const plural = {
    articles: 'ARTICLE',
